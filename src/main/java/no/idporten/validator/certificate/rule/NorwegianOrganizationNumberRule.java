...
import no.idporten.validator.certificate.api.PrincipalNameProvider;
import no.idporten.validator.certificate.api.Property;
import no.idporten.validator.certificate.api.Report;
import no.idporten.validator.certificate.util.SimpleProperty;
...
public class NorwegianOrganizationNumberRule extends PrincipalNameRule {
    ...
    public static NorwegianOrganization extractNumber(X509Certificate certificate) throws CertificateValidationException {
        ...
        try {
            ...
            //matches "2.5.4.97=PSDNO-FSA-991825827,CN=DIGITALISERINGSDIREKTORATET,OU=FDF Test PSD2 QC,O=DIGITALISERINGSDIREKTORATET,C=NO"
            //matches "2.5.4.97=NTRNO-991825827,CN=DIGITALISERINGSDIREKTORATET,OU=FDF Test PSD2 QC,O=DIGITALISERINGSDIREKTORATET,C=NO"
            //matches "2.5.4.97=991825827,CN=DIGITALISERINGSDIREKTORATET,OU=FDF Test PSD2 QC,O=DIGITALISERINGSDIREKTORATET,C=NO"
            for (String value : extract(getSubject(certificate), "2.5.4.97")) {
                Matcher matcher = patternOrganizationName.matcher(value);
                if (matcher.matches()) {
                    return new NorwegianOrganization(matcher.group(1), name.get(0));
                } else {
                    matcher = patternSerialNumber.matcher(value);
                    if (matcher.matches()) {
                        return new NorwegianOrganization(value, name.get(0));
                    }
                }
            }
